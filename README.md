...
def deleteDuplicates(head):
    ...
    cur = head
    while cur: # while cur exists
    # while cur.next exists AND cur and cur.next 
    # are of the same value ,delete
    # ok its .data not .val here 
        while cur.next and cur.next.data == cur.data:
            # skipparoo
            cur.next = cur.next.next
        # set cur to cur.next is non equal
        cur = cur.next
